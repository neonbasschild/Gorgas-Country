...
cdata =
{
  ...
  save_mission_failed_msg = _('Can not open file, recording is not possible '),
...
local function onTasksRemoveFromOtherGroups(a_group)
	local bEnable = false
...
--print('load(fName)',fName)
    ...
    panel_ship.updateCountries()
    panel_vehicle.updateCountries()
    panel_static.updateCountries()
							...
							for s,r in pairs(u.units) do
								if me_db.unit_by_type[r.type] then
									...
									if not r.name then
										...
										r.name = check_unit_name('Unit #001')
									end
...
--base.print("---create_new_mission----")
  ...
  targetNameIndex = 0
  zoneNameIndex = 0
  mission.date = { Year = 2011, Month = 6, Day = 01 }
...
function unload_static_groups(u, type, cant, um)
  ...
  for k,w in pairs(u[type].group) do
    mod_dictionary.textToMis(w, 'name', w.name, w.KeyDict_name)    
    local categoryUnit = crutches.staticCategoryNameToId(base.panel_static.getCategory(w.units[1].type))
    ...
    if w.route.points[1].linkUnit then
		...
		if w.linkOffset then -- записываем таблицу смещения и поворотов относительно родителя повернутого в 0
			local unitP = base.module_mission.unit_by_id[pt.linkUnit]
			gr.units[1].offsets = {}
...
function convertStartTimeToDate(start_time)
	...
	local MissionDate = { Year = 2011, Month = 6, Day = 01 }
...
-------------------------------------------------------------------------------
-- Проверка имени группы на уникальность и формирование уникального имени.
...
-------------------------------------------------------------------------------
-- Проверка имени юнита на уникальность и формирование уникального имени.
...
function createUnitZones(unit)
	...
	if udb.category ~= 'Air Defence' and (not enableInSelected) then
		return
	end
...
function update_target_zone(target)
	...
	local zone = table.remove(target.boss.boss.mapObjects.route.targetZones[target.boss.index], target.index)
		if zone then
			--mapObjects[target.boss.boss.mapObjects.route.targetZones[target.boss.index][target.index].id] = nil -- бред!!! элемент уже удалён. будет падать!
...
function scale_group_map_objects(group, scale, objects, toRemove)
  ...
  for i=2,#units do -- просматриваем все юниты начиная со второго,т.к. первый - точка маршрута со значком юнита
    ...
    if r then -- есть зона, рисуем по любому на всех масштабах
    ...
    elseif 	'cyclone' == group.type then
		...
		if group.mapObjects and (group.mapObjects.units) then		
			for i = 2, #group.mapObjects.units do -- начинаем со второго юнита т.к. вместо первого        
				...
				if hasZone then -- если есть зона рисуем юнит всегда на любом масштабе
...
function saveTrack(fileName)
    ...
    if file then
		...
		showWarningMessageBox(_('Can not open file, recording is not possible ') .. trackPath)
...
--print("createAirportEquipment()")
    ...
    function fillWarehouse()
	    ...
	    for k,v in pairs(base.db.Units.Planes.Plane) do
            tmpAirportEquipment.aircrafts.planes[v.type] = {initialAmount    = AEDefault.LA.initialAmount,
															wsType           = {v.attribute[1],v.attribute[2],v.attribute[3],v.attribute[4]}}
        end
        
        for k,v in pairs(base.db.Units.Helicopters.Helicopter) do    
            tmpAirportEquipment.aircrafts.helicopters[v.type] = {initialAmount    = AEDefault.LA.initialAmount,
																 wsType           = {v.attribute[1],v.attribute[2],v.attribute[3],v.attribute[4]}}
        end
